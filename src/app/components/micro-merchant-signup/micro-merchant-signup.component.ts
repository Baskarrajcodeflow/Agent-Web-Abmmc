...
@Component({
  selector: 'app-micro-merchant-signup',
  standalone: true,
  imports: [ReactiveFormsModule, CommonModule, LoaderComponent, FormsModule],
  templateUrl: './micro-merchant-signup.component.html',
  styleUrl: './micro-merchant-signup.component.scss'
})
export class MicroMerchantSignupComponent {
 isLoading: boolean = false;
   ...
   createPayload() {
     ...
     this.apiService.b2bSignIp(request).subscribe({
       next: (res) => {
         if (res) {
           alert('Micro-Merchant Registration Success')
